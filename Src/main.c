...
double get_adc_value(ADC_Data *data);									// Get ADC2 Channel 3 data, converted to voltage
...
uint32_t adc_buffer[32];												// Buffer used for ADC2 DMA interrupt
...
//		}
		...
		if (adc_conversion_done == true) {
			...
			for (int x = 0; x < 16; x++) {
				...
				adc_data.adc_value_vrefint_channel += *tempbuf++;
			}

			adc_data.adc_value_channel_3 = adc_data.adc_value_channel_3 >> 4;
			adc_data.adc_value_vrefint_channel = adc_data.adc_value_vrefint_channel >> 4;

			double adc_result = (double) (3.3 * adc_data.adc_value_vrefint_register * adc_data.adc_value_channel_3) / (adc_data.adc_value_vrefint_channel * 4095);

			HAL_ADC_Start_DMA(&hadc2, (uint32_t*)adc_buffer, 32);
		}
